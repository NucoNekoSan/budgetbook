...
- 上6つは Django 標準の機能、下2つがこのプロジェクト固有
...
### _build_monthly_trend() — 12ヶ月推移データの構築
...
def _build_monthly_trend(target_month: date) -> list[dict]:
    ...
    rows = (
        ...
        .values('month')
        .annotate(
            income=Coalesce(Sum('amount', filter=Q(category__kind='income')), Value(0)),
            expense=Coalesce(Sum('amount', filter=Q(category__kind='expense')), Value(0)),
        )
        .order_by('month')
    )
```
...
**Django ORM の高度なクエリ:**
- `TruncMonth('date')`: 日付を月単位に切り捨て（2026-04-15 → 2026-04-01）
- `.values('month')`: 月ごとにグループ化
- `Sum('amount', filter=Q(...))`: 条件付き合計（収入だけ、支出だけを集計）
- `Coalesce(..., Value(0))`: 合計が NULL（データなし）の場合は 0 を返す
- これにより1回のSQLクエリで12ヶ月分の月別収支データを取得
...
- 取引がない月はデータベースから返ってこない
- 全12ヶ月のデータを確実に用意するため、欠落月は0で埋める
...
- `defer`: スクリプトをHTML解析後に実行（ページ表示を妨げない）
...
- `htmx.onLoad`: HTMX が新しいコンテンツを読み込んだ後にグラフを初期化
